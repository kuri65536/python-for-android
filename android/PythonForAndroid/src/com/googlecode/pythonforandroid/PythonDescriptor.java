...
public class PythonDescriptor extends Sl4aHostedInterpreter {
  ...
  private static final String PYTHON_BIN = "bin/python.so";
  ...
  public Map<String, String> getEnvironmentVariables(Context context) {
    ...
    values.put(ENV_LD, new File(context.getFilesDir().getParentFile(), "lib").getAbsolutePath());
    values.put(ENV_PATH, new File(context.getFilesDir().getParentFile(),
        "/lib/libpython_extras.so/python") + ":" + getHome(context) + "/lib/");
    values.put(ENV_EGGS, getHome(context) + "/lib/");
